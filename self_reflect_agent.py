...
st.title("🧠 自我反思纠错 Agent")
st.subheader("先作答 → 自查错误 → 自主修正 → 输出精准答案")

user_q = st.text_input("输入问题","解释清楚牛顿三大定律")

if st.button("启动智能体思考修正") and user_q:
    with st.spinner("智能体正在思考+自查修正..."):
        result = reflect_agent.invoke({"query":user_q})
    
    st.markdown("### 1. 初次原始回答")
    st.info(result["raw_answer"])
    
    st.markdown("### 2. 智能体自我反思纠错")
    st.warning(result["reflect_opinion"])
    
    st.markdown("### 3. 修正后最终标准答案")
